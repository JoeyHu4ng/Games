...
class Scoreboard:
    ...
    def __init__(self, scoreboard={key: [] for key in ["Beginner", "Intermediate", "Expert"]}):
        self.mater = tk.Tk()
    ...
    def __show_data(self, level="Beginner"):
        ...
        for i in range(10):
            rank = tk.Label(self.mater, height=1, width=3, text="No. %d" % (i + 1))
            rank.grid(row=i, column=0, columnspan=3, sticky=tk.W + tk.E + tk.N + tk.S)
            ...
            name_label.grid(row=i, column=3, columnspan=20, sticky=tk.W + tk.E + tk.N + tk.S)
            grade_label = tk.Label(self.mater, height=1, width=6, text=grade)
            grade_label.grid(row=i, column=23, columnspan=6, sticky=tk.W + tk.E + tk.N + tk.S)
